const {getAllReviews,createReview,getUserReviews,deletReview, editReview} = require("../Controllers/reviewsController.js");
...
const getReviewByUser = async (req,res) =>{
  try {
    if (!req.body) { return res.status(400).json({ error: "Datos del body incorrectos o unsuficientes" });}
...
const deleteReviewById = async (req, res) => {
  try {
    ...
    if (!id) { return res.status(400).json({ error: "Parametros incorrectos o unsuficientes." });}

    const deletCount = await deletReview(id);
    if(deletCount > 0){ 
      res.status(200).json({msg: `La review con el id ${id}, fue eliminada satisfactoriamente.` });}
   
  } catch (error) {
    res.status(500).json({ msg: "Error al intentar eliminar la review.",error });
  };
};
...
const editRviewComment = async (req, res) => {
  try {
    ...
    res.status(201).json({msg: `Los comentarios fueron editados satisfactoriamente.`,editedReview});
